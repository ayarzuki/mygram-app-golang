...
import (
	"fmt"
	"mygram-app-golang/controllers"
	"mygram-app-golang/dbconfig"
	"mygram-app-golang/middleware"
	"mygram-app-golang/repositories"
	"mygram-app-golang/services"
...
func main() {
	// Inisialisasi DB
	db, err := dbconfig.ConnectDB(dbconfig.GoDotEnvVariable("DB_DRIVER"))
	if err != nil {
	...
	route.Run(dbconfig.GoDotEnvVariable("APP_PORT"))
}
